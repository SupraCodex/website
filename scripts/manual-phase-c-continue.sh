...
ROOT="${1:?root de phase C}"; id="${2:?article id}"
set -a; . ./.env; set +a
...
api(){ name=$1; expected=$2; method=$3; path=$4; token=$5; shift 5; if [ -n "$token" ]; then body=$(curl -sS -X "$method" -w '\n__STATUS__%{http_code}' -H "Authorization: Bearer $token" "$@" "$BASE$path"); else body=$(curl -sS -X "$method" -w '\n__STATUS__%{http_code}' "$@" "$BASE$path"); fi; status=${body##*__STATUS__}; body=${body%__STATUS__*}; printf '%-38s %s (attendu %s)\n' "$name" "$status" "$expected"; test "$status" = "$expected"; printf '%s' "$body" > "$ROOT/$name.json"; }
api C19-patch-draft 200 PATCH "/api/v1/admin/articles/$id" "$editor" -H 'Content-Type: application/json' -d "{\"type\":\"article\",\"title\":\"Article recette finale v2\",\"excerpt\":\"Résumé v2.\",\"slug\":\"recette-reprise-${id}-v2\",\"body\":[{\"type\":\"p\",\"text\":\"Contenu v2\"}],\"categoryId\":null,\"tagIds\":[]}"
api C20-patch-reviewer-403 403 PATCH "/api/v1/admin/articles/$id" "$reviewer" -H 'Content-Type: application/json' -d '{"type":"article","title":"x","excerpt":"x","slug":"x","body":[],"categoryId":null,"tagIds":[]}'
api C21-submit-editor 200 POST "/api/v1/admin/articles/$id/submit" "$editor"
api C22-reject-editor-403 403 POST "/api/v1/admin/articles/$id/reject" "$editor"
api C23-reject-reviewer 200 POST "/api/v1/admin/articles/$id/reject" "$reviewer"
...
preview=$(jq -r .data.token "$ROOT/C27-preview-editor.json")
...
media=$(jq -r .data.id "$ROOT/C34-media-upload-editor.json")
api C35-media-upload-reviewer-403 403 POST /api/v1/admin/media "$reviewer" -H 'Content-Type: application/json' -d "$media_body"
api C36-media-bad-extension 422 POST /api/v1/admin/media "$editor" -H 'Content-Type: application/json' -d '{"filename":"note.exe","contentBase64":"iVBORw0KGgo=","mimeType":"image/png","altText":"x"}'
api C37-media-bad-mime 422 POST /api/v1/admin/media "$editor" -H 'Content-Type: application/json' -d '{"filename":"doc.png","contentBase64":"iVBORw0KGgo=","mimeType":"application/pdf","altText":"x"}'
...
api C46-article-patch-cover 200 PATCH "/api/v1/admin/articles/$id" "$editor" -H 'Content-Type: application/json' -d "{\"type\":\"article\",\"title\":\"Article publiable\",\"excerpt\":\"Résumé publiable.\",\"slug\":\"recette-reprise-${id}-v3\",\"body\":[{\"type\":\"p\",\"text\":\"Couverture fournie\"}],\"coverMediaId\":\"$media\",\"categoryId\":null,\"tagIds\":[]}"
...
printf 'PHASE C CONTINUE PASSED\n'
